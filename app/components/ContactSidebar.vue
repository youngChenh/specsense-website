<template>
  <div class="fixed right-4 top-1/2 -translate-y-1/2 z-40 flex flex-col gap-3" :style="{ marginTop: '145px' }">
      ...
      >
        ...
        <div class="flex flex-col items-center">
          <div class="w-40 h-40 bg-gray-100 rounded-lg flex items-center justify-center mb-4 overflow-hidden">
            <img src="/whatsapp.jpg" alt="WhatsApp QR Code" class="w-full h-full object-contain cursor-zoom-in" @click="whatsappLightboxOpen = true" />
          </div>
          <div class="w-full space-y-3">
            ...
            <a
              ...
              target="_blank"
              class="w-full inline-flex items-center justify-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors text-sm font-medium"
            >
    ...
    <Teleport to="body">
      <div
        v-if="whatsappLightboxOpen"
        class="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center"
        @click="whatsappLightboxOpen = false"
      >
        <button
...
const whatsappLightboxOpen = ref(false)
...
watch(whatsappLightboxOpen, (open) => {
  if (open) {
    document.addEventListener('keydown', handleLightboxKeydown)
  } else {
    document.removeEventListener('keydown', handleLightboxKeydown)
...
function handleLightboxKeydown(e: KeyboardEvent) {
  if (e.key === 'Escape') {
    wechatLightboxOpen = false
    whatsappLightboxOpen = false
  }
}
</script>
